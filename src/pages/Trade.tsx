...
import { ArrowUpDown, Clock, LayoutDashboard, List, Search, TrendingUp, TrendingDown, RefreshCw, Activity, Zap } from 'lucide-react';
...
export default function Trade() {
  ...
  useEffect(() => { if (mode === 'mt5') { fetchSignals(); const interval = setInterval(fetchSignals, 10000); return () => clearInterval(interval); } }, [mode, fetchSignals]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className={`text-xs md:text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              {mode === 'classic' ? `${t('paperTrading')} • ${t('demoAccount')}` : (isBn ? 'MT5 স্টাইল মার্কেট ওয়াচ ও চার্ট' : 'MT5-style Market Watch & Charts')}
            </p>
          </div>
